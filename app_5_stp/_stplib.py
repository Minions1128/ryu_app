...
class Stp(app_manager.RyuApp):
    ...
    def __init__(self):
        ...
        self._set_logger()
        self.config = {}
        # init see self.set_config()
        self.bridge_list = {}
        # {<dpid>:<class Bridge>}

    def close(self):
        # close stp, unregister all bridge.
        for dpid in self.bridge_list:
            self._unregister_bridge(dpid)

    def _set_logger(self):
        # start set logger when stp is executing.
    ...
    def dispacher_change(self, ev):
        # notification bridge up or down and its reasons.
    ...
    def _register_bridge(self, dp):
        # new a Bridge class, and add it to the bridge_list
    ...
    def packet_in_handler(self, ev):
        # Packet in Event, call Bridge.packet_in_handler() method
    ...
    def port_status_handler(self, ev):
        # Add, delete or modify port.
        # 1.Add, execute bridge.port_add method.
        # 2.Delete, execute bridge.port_delete method.
        # 3.Modify, bridge.link_down, bridge.link_up
        dp = ev.msg.datapath
        dpid_str = {'dpid':dpid_to_str(dp.id)}
        port = ev.msg.desc
    ...
    def compare_bpdu_info(my_priority, my_times, rcv_priority, rcv_times):
        ...
        if my_priority is None:
            ...
            if not result:
                ...
                if not result:
                    # the received info is from the same root bridge
    ...
    def _cmp_obj(obj1, obj2):
        # obj1 is new, obj2 is old.
        # check if there is attribute or value changed in new obj
...
class Bridge(object):
    _DEFAULT_VALUE = {
        'priority':bpdu.DEFAULT_BRIDGE_PRIORITY,
        # DEFAULT_BRIDGE_PRIORITY = 32768
        'sys_ext_id':0,
        'max_age':bpdu.DEFAULT_MAX_AGE,
        # DEFAULT_MAX_AGE = 20, the max age of keeping bpdu info 
        'hello_time':bpdu.DEFAULT_HELLO_TIME,
    ...
    def __init__(self, dp, logger, config, send_ev_func):
        ...
        # app_manager.RyuApp.send_event_to_observers
    ...
    def packet_in_handler(self, msg):
        """ According to the type of received packets,
            Ee will execute different operations.
            1. If Received Configuration BPDU.
            ...
            2. Received Topology Change Notification BPDU.
        ...
        if bpdu.ConfigurationBPDUs in pkt:
			...
			# if bpdu is updated
			# if bpdu include topology change info
    ...
    def topology_change_notify(self, port_state):
        ...
        if notice:
            ...
            # send bpdu which set tc-bit to all bridge via DP
                self._transmit_tc_bpdu()
            else:
                # send tcn bpdu to root bridge via root port
...
class Port(object):
    _DEFAULT_VALUE = {
        'priority':bpdu.DEFAULT_PORT_PRIORITY,  # 128
        'path_cost':bpdu.PORT_PATH_COST_10MB,   # 2000000
        'enable':True
    }

    def __init__(self, dp, logger, config, send_ev_func, timeout_func, topology_change_func, bridge_id, bridge_times, ofport):
        ...
        values = self._DEFAULT_VALUE
        path_costs = {
            dp.ofproto.OFPPF_10MB_HD:bpdu.PORT_PATH_COST_10MB,
        ...
        self.state = (None if self.config_enable
    ...
    def rcv_config_bpdu(self, bpdu_pkt):
        ...
        rcv_info = Stp.compare_bpdu_info(self.designated_priority,
            self.designated_times, msg_priority, msg_times)
        if rcv_info is SUPERIOR:
    ...
    def _generate_config_bpdu(self, flags):
        ...
        l = llc.llc(llc.SPA_BPDU, llc.SPA_BPDU, llc.ControlFormatU())
...
class PortId(object):
    def __init__(self, priority, port_no):
        ...
        self.value = bpdu.ConfigurationBPDUs.encode_port_id(
            priority, port_no)


class Priority(object):
    def __init__(self, root_id, root_path_cost, designated_bridge_id, designated_port_id):
...
class Times(object):
    def __init__(self, message_age, max_age, hello_time, forward_delay):
        ...
        self.max_age = max_age # 20
        self.hello_time = hello_time # 2
        self.forward_delay = forward_delay # 15
...
class OfCtl_v1_0(object):
    ...
    def set_port_status(self, port, state):
        ...
        mask = 0b1111111
        msg = ofproto_parser.OFPPortMod(self.dp, port.port_no,
            port.hw_addr, PORT_CONFIG_V1_0[state], mask,
            port.advertised)
...
class OfCtl_v1_2later(OfCtl_v1_0):
    ...
    def set_port_status(self, port, state):
        ...
        mask = 0b1100101
        msg = parser.OFPPortMod(self.dp, port.port_no, 
            port.hw_addr, config[ofp][state], mask,
            port.advertised)
        self.dp.send_msg(msg)
